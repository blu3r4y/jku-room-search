...
export class Logger {
     ...
     */
    public static info(text: any, category?: string, status?: string, progress?: number, error: boolean = false) {
        if (category == null && status == null && progress == null) {
            ...
            status = error ? colors.brightRed(`${status}`) : colors.brightGreen(`${status}`);

            let progressStr = (progress == null ? "" : Math.round(progress * 100).toString() + "%").padStart(4, " ");
