export class SplitTree {
    ...
    /**
     * Given a full interval and a set of exclusions to cut out of it, return a minimal set of remaining intervals
     *
    ...
    private static appendLeafs(interval: Interval, list: Array<[number, number]>) {
        ...
        if (interval.left == null && interval.right == null) {
            ...
            if (interval.left != null) { this.appendLeafs(interval.left, list); }
            if (interval.right != null) { this.appendLeafs(interval.right, list); }
        }
    }
}
...
class Interval {
    ...
    public split(x: number, y: number) {
        ...
        if (this.left != null || this.right != null) {
            ...
            if (this.left != null && x < this.left.b) { this.left.split(x, y); }
            if (this.right != null && y > this.right.a) { this.right.split(x, y); }
        } else {
